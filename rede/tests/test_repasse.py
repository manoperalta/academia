...
def regra_padrao(db, rede):
    return RegraDeRepasse.objects.create(
        ...
        base=RegraDeRepasse.Base.BRUTO,
        fundo_de_marketing=Decimal("2.00"),
        piso_minimo=Decimal("0.00"),
    )
...
def test_base_liquida_exclui_taxas_e_estornos(db, rede, unidades, receita):
    RegraDeRepasse.objects.create(
        ...
        percentual_de_taxas_de_gateway=Decimal("3.00"),
        excluir_taxas_de_gateway=True,
        excluir_estornos=True,
    )
    inicio, fim = periodo_do_mes()
    calculo = calcular_repasse(unidades[0], inicio, fim)
    ...
    exclusoes = [
        linha for linha in calculo["linhas"] if linha["tipo"] == ItemDeRepasse.Tipo.EXCLUSAO
    ]
...
def test_piso_minimo_ajusta_o_total(db, rede, unidades, receita):
    RegraDeRepasse.objects.create(
        ...
        unidade=None,
        tipo=RegraDeRepasse.Tipo.PERCENTUAL,
        percentual=Decimal("1.000"),
        piso_minimo=Decimal("100.00"),
    )
    inicio, fim = periodo_do_mes()
    calculo = calcular_repasse(unidades[0], inicio, fim)
...
def test_regra_da_unidade_tem_prioridade(db, rede, unidades, receita, regra_padrao):
    RegraDeRepasse.objects.create(
        rede=rede, unidade=unidades[0], tipo=RegraDeRepasse.Tipo.FIXO, valor_fixo=Decimal("500.00")
    )
...
def test_conferencia_detecta_valor_alterado(db, rede, unidades, receita, regra_padrao):
    ...
    assert any(
        divergencia["linha"] == "Total devido" for divergencia in conferencia["divergencias"]
    )
