...
def cliente_da_unidade(db, rede, unidades):
    ...
    VinculoUsuario.todos.create(
        usuario=usuario, rede=rede, unidade=unidades[0], papel=Papel.GESTOR_UNIDADE, ativo=True
    )
    cliente = Client()
...
def receita(db, rede, unidades):
    ...
    plano = Plano.objects.create(
        rede=rede, unidade=unidades[0], nome="Mensal", tipo="mensal", valor=Decimal("150.00")
    )
    plano_rede = Plano.objects.create(
        rede=rede, unidade=None, nome="Mensal Rede", tipo="mensal", valor=Decimal("180.00")
    )
    ...
    for indice, unidade in enumerate(unidades):
        for posicao in range(3 + indice):
            login = get_user_model().objects.create_user(
                username=f"aluno{indice}{posicao}",
                password=SENHA,
                email=f"a{indice}{posicao}@x.com",
            )
