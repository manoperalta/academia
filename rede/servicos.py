...
def validar_desconto(unidade: Unidade, percentual) -> dict:
    ...
    if (
        not politica_da_rede.trava_politica_de_desconto
        or percentual <= politica_da_rede.teto_de_desconto
    ):
        return {
            "permitido": True,
...
def receita_do_periodo(unidade: Unidade, inicio: date, fim: date) -> dict:
    ...
    devolvidos = _dinheiro(
        por_unidade.filter(status__in=["devolvido", "cancelado"]).aggregate(
            total=Sum("valor_pago")
        )["total"]
    )
    parceiros = ZERO
    detalhe_por_plano = list(
        pagos.values("plano__nome")
        .annotate(total=Sum("valor_pago"), quantidade=Count("id"))
        .order_by("-total")
    )
...
def calcular_repasse(unidade: Unidade, inicio: date, fim: date) -> dict:
    ...
    if regra.base == RegraDeRepasse.Base.LIQUIDO:
        if regra.excluir_estornos and receita["devolucoes"]:
            ...
            linhas.append(
                {
                    "ordem": ordem,
                    "tipo": ItemDeRepasse.Tipo.EXCLUSAO,
                    "descricao": "Estornos e devolucoes",
                    "base": receita["devolucoes"],
                    "percentual": ZERO,
                    "valor": -receita["devolucoes"],
                    "referencia": "",
                }
            )
            ordem += 1
            ...
            linhas.append(
                {
                    ...
                    "base": receita["receita_bruta"],
                    "percentual": regra.percentual_de_taxas_de_gateway,
                    "valor": -taxa,
                    "referencia": "",
                }
            )
            ...
            linhas.append(
                {
                    ...
                    "base": receita["receita_bruta"],
                    "percentual": regra.percentual_de_impostos,
                    "valor": -impostos,
                    "referencia": "",
                }
            )
            ...
            linhas.append(
                {
                    ...
                    "base": receita["planos_de_parceiros"],
                    "percentual": ZERO,
                    "valor": -receita["planos_de_parceiros"],
                    "referencia": "",
                }
            )
            ordem += 1
    ...
    base_de_calculo = (
        receita_liquida if regra.base == RegraDeRepasse.Base.LIQUIDO else receita["receita_bruta"]
    )
    ...
    if regra.tipo in {RegraDeRepasse.Tipo.PERCENTUAL, RegraDeRepasse.Tipo.PERCENTUAL_MAIS_FIXO}:
        ...
        linhas.append(
            {
                "ordem": ordem,
                "tipo": ItemDeRepasse.Tipo.ROYALTY,
    ...
    if regra.tipo in {RegraDeRepasse.Tipo.FIXO, RegraDeRepasse.Tipo.PERCENTUAL_MAIS_FIXO}:
        ...
        linhas.append(
            {
                "ordem": ordem,
                "tipo": ItemDeRepasse.Tipo.FIXO,
                "descricao": "Valor fixo do periodo",
                "base": ZERO,
                "percentual": ZERO,
                "valor": valor_fixo,
                "referencia": regra.get_tipo_display(),
    ...
    if regra.fundo_de_marketing:
        ...
        linhas.append(
            {
                "ordem": ordem,
                "tipo": ItemDeRepasse.Tipo.FUNDO,
                "descricao": f"Fundo de marketing de {regra.fundo_de_marketing}%",
                "base": base_de_calculo,
                "percentual": regra.fundo_de_marketing,
                "valor": fundo,
                "referencia": "",
            }
        )
            ...
            "por_plano": [
                {
                    ...
                    "total": str(_dinheiro(linha["total"])),
...
@transaction.atomic
def emitir_repasse(
    unidade: Unidade, inicio: date, fim: date, usuario=None, forcar: bool = False
) -> Repasse:
    ...
    repasse.memoria = {
        ...
        "regra": {
            "tipo": calculo["regra"].get_tipo_display(),
            "base": calculo["regra"].get_base_display(),
...
def conferir_repasse(repasse: Repasse) -> dict:
    ...
    linhas_novas = [
        (linha["tipo"], linha["descricao"], _dinheiro(linha["valor"]))
        for linha in recalculado["linhas"]
    ]
    ...
    if len(linhas_gravadas) != len(linhas_novas):
        divergencias.append(
            {
                "linha": "Quantidade de linhas da memoria",
...
def relatorio_de_repasses(rede, inicio: date, fim: date) -> dict:
    repasses = Repasse.objects.filter(rede=rede, inicio__gte=inicio, fim__lte=fim).select_related(
        "unidade"
    )
    return {
        "repasses": repasses,
        ...
        "por_unidade": list(
            repasses.values("unidade__nome")
            .annotate(devido=Sum("valor_devido"), pago=Sum("valor_pago"))
            .order_by("-devido")
        ),
...
def comparativo_entre_unidades(rede, inicio: date, fim: date) -> list[dict]:
    ...
    for unidade in unidades:
        ...
        recebido = _dinheiro(
            pagamentos.filter(status="pago").aggregate(total=Sum("valor_pago"))["total"]
        )
        em_aberto = _dinheiro(
            pagamentos.filter(status__in=["pendente", "em_analise"]).aggregate(
                total=Sum("valor_pago")
            )["total"]
        )
        alunos_ativos = Usuario.todos.filter(
            rede=rede, unidade=unidade, status_user="Ativo"
        ).count()
        alunos_novos = Usuario.todos.filter(
            rede=rede,
            unidade=unidade,
            data_create_user__date__gte=inicio,
            data_create_user__date__lte=fim,
        ).count()
...
def consolidado_da_rede(rede, inicio: date, fim: date) -> dict:
    ...
    por_mes = list(
        Pagamento.objects.filter(
            rede=rede, status="pago", data_pagamento__gte=inicio, data_pagamento__lte=fim
        )
    ...
    return {
        ...
        "ticket_medio": _dinheiro(
            total_recebido / sum(linha["alunos_ativos"] for linha in comparativo)
        )
        if sum(linha["alunos_ativos"] for linha in comparativo)
        else ZERO,
        "por_mes": por_mes,
...
def calcular_realizado(meta: Meta) -> Decimal:
    ...
    pagamentos = Pagamento.objects.filter(
        ...
        data_pagamento__gte=meta.inicio,
    ...
    if meta.indicador == Meta.Indicador.NOVAS_MATRICULAS:
        return Decimal(
            Usuario.todos.filter(
                rede=meta.rede,
                unidade__in=unidades,
                data_create_user__date__gte=meta.inicio,
    ...
    if meta.indicador == Meta.Indicador.TICKET_MEDIO:
        ...
        alunos = Usuario.todos.filter(
            rede=meta.rede, unidade__in=unidades, status_user="Ativo"
        ).count()
    ...
    if meta.indicador == Meta.Indicador.RETENCAO:
        ...
        hoje_ativos = Usuario.todos.filter(
            ...
            unidade__in=unidades,
            status_user="Ativo",
            data_create_user__date__lte=meta.inicio,
        ).count()
...
def distribuir_catalogo(rede, referencia: str, unidades, usuario=None) -> DistribuicaoDeCatalogo:
    ...
    origem = (
        Aula.todos.filter(rede=rede, pk=int(referencia)).first()
            ...
            try:
                ...
                for campo in origem._meta.concrete_fields:
                    if campo.name in {"id", "pk", "rede", "unidade"} or campo.primary_key:
                        continue
                    dados[campo.name] = (
                        None if campo.name == "arquivado_em" else getattr(origem, campo.name)
                    )
    ...
    distribuicao = DistribuicaoDeCatalogo.objects.create(
        ...
        unidades=[unidade.pk for unidade in unidades],
        resultado=resultado,
...
def aplicar_template(
    unidade: Unidade, template: TemplateDeUnidade | None, usuario=None
) -> ImplantacaoDeUnidade:
    ...
    implantacao, _ = ImplantacaoDeUnidade.objects.update_or_create(
        unidade=unidade,
        defaults={"template": template, "resultado": resultado},
    )
    from api.auditoria import registrar
...
@transaction.atomic
def marcar_item_do_checklist(
    implantacao: ImplantacaoDeUnidade, item_id: int, concluido: bool = True
):
...
def transferir_alunos(alunos, destino: Unidade, motivo: str = "", usuario=None) -> dict:
    ...
    for aluno in alunos:
        ...
        transferencias.append(
            TransferenciaDeAluno.objects.create(
                rede=destino.rede,
                aluno=aluno,
                origem=origem,
                destino=destino,
                motivo=motivo[:200],
                autorizado_por=usuario if getattr(usuario, "pk", None) else None,
                lote=lote,
            )
        )
    from api.auditoria import registrar
...
def encerrar_unidade(unidade: Unidade, destino: Unidade | None, usuario=None) -> dict:
    ...
    if destino is not None:
        movimento = transferir_alunos(
            alunos, destino, motivo=f"Encerramento de {unidade.nome}", usuario=usuario
        )
    ...
    registro = Comunicado.objects.create(
        rede=unidade.rede,
        unidade=None,
        publico=Comunicado.Publico.UNIDADES,
        titulo=f"Unidade {unidade.nome} encerrada",
        ...
        mensagem=(
            ...
            + (
                ...
                if destino
                else "Os alunos ficaram sem unidade de destino."
            )
...
def importar_multi_unidade(
    conteudo: bytes, rede, tipo: str = "alunos", atualizar: bool = False
) -> dict:
