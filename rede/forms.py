...
from rede.models import (
    Comunicado, ItemDeChecklist, Meta, PoliticaDaRede, RegraDeRepasse, SolicitacaoDeAprovacao,
    TemplateDeUnidade,
)
...
class UnidadeForm(EstiloDaRedeMixin, forms.ModelForm):
    class Meta:
        model = Unidade
        fields = ["nome", "codigo", "tipo", "cnpj", "endereco", "cidade", "uf", "telefone",
                  "gestor", "status", "sobrescrever_branding"]
    ...
    def __init__(self, *args, rede=None, **kwargs):
        ...
        if rede is not None:
            ...
            self.fields["gestor"].queryset = get_user_model().objects.filter(
                vinculos__rede=rede, vinculos__ativo=True).distinct()
...
class MetaForm(EstiloDaRedeMixin, forms.ModelForm):
    ...
    def __init__(self, *args, rede=None, **kwargs):
        ...
        self.fields["fim"].initial = self.fields["fim"].initial or (
            hoje.replace(day=1) + timedelta(days=32)).replace(day=1) - timedelta(days=1)


class RegraDeRepasseForm(EstiloDaRedeMixin, forms.ModelForm):
    class Meta:
        model = RegraDeRepasse
        fields = ["unidade", "tipo", "percentual", "valor_fixo", "base",
                  "excluir_taxas_de_gateway", "percentual_de_taxas_de_gateway",
                  "excluir_estornos", "excluir_impostos", "percentual_de_impostos",
                  "excluir_planos_de_parceiros", "fundo_de_marketing", "piso_minimo",
                  "dia_de_vencimento", "inicio_da_vigencia", "fim_da_vigencia", "ativo"]
        widgets = {"inicio_da_vigencia": forms.DateInput(attrs={"type": "date"}),
    ...
    def clean(self):
        ...
        if dados.get("tipo") in {RegraDeRepasse.Tipo.FIXO, RegraDeRepasse.Tipo.PERCENTUAL_MAIS_FIXO} \
...
class TemplateDeUnidadeForm(EstiloDaRedeMixin, forms.ModelForm):
    class Meta:
        ...
        widgets = {"configuracoes": forms.Textarea(attrs={
            "rows": 6,
...
class DistribuicaoForm(EstiloDaRedeMixin, forms.Form):
    ...
    unidades = forms.ModelMultipleChoiceField(
        label="Distribuir para", queryset=Unidade.objects.none(),
...
class AprovacaoDecisaoForm(EstiloDaRedeMixin, forms.Form):
    justificativa = forms.CharField(label="Justificativa", widget=forms.Textarea(attrs={"rows": 3}),
                                    required=False)


class ImportacaoDaRedeForm(EstiloDaRedeMixin, forms.Form):
    tipo = forms.ChoiceField(label="O que importar", choices=[("alunos", "Alunos"),
