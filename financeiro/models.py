...
class GatewayConfig(TenantModel):
    ...
    ambiente = models.CharField(
        max_length=20, choices=AMBIENTE_CHOICES, default="sandbox", verbose_name="Ambiente"
    )
    ...
    access_token = models.CharField(
        ...
        help_text="Token de acesso do gateway",
    )
    public_key = models.CharField(
        ...
        default="",
        blank=True,
        help_text="Chave pública para checkout",
    )

    # Configurações adicionais
    url_webhook = models.URLField(
        verbose_name="URL de Notificação (Webhook)",
        blank=True,
        null=True,
        help_text="URL para receber atualizações de status",
    )
...
class Plano(TenantModel):
    ...
    imagem = models.ImageField(
        upload_to="planos/", blank=True, null=True, verbose_name="Imagem de Capa"
    )
...
class Pagamento(TenantModel):
    ...
    transaction_id = models.CharField(
        max_length=100, blank=True, null=True, verbose_name="ID da Transação (Gateway)"
    )
