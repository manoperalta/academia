...
def pagamento_create(request):
    ...
    return render(
        request, "financeiro/pagamento_form.html", {"form": form, "title": "Registrar Pagamento"}
    )
...
def realizar_checkout(request, pk):
    ...
    if pagamento.usuario != request.user and not (
    ...
    try:
        ...
        messages.error(request, f"Erro ao gerar pagamento: {e!s}")
        return redirect("pagamento_list")
...
def comprar_plano(request, pk):
    ...
    pagamento_pendente = Pagamento.objects.filter(
        usuario=request.user, plano=plano, status="pendente"
    ).first()
...
def atualizar_status_pagamento(request, pk):
    ...
    if request.method == "POST":
        ...
        if novo_status in dict(Pagamento.STATUS_CHOICES):
            ...
            messages.success(
                request,
                f"Status do pagamento de {pagamento.usuario} atualizado para {pagamento.get_status_display()}.",
            )
