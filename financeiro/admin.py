...
from .models import Despesa, GatewayConfig, Pagamento, Plano
...
class GatewayConfigAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Configuração Geral", {"fields": ("gateway", "ativo", "ambiente")}),
        (
            "Credenciais",
            {
...
class PagamentoAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Informações do Pagamento", {"fields": ("usuario", "plano", "valor_pago", "status")}),
        ("Vigência", {"fields": ("data_inicio", "data_fim")}),
        (
            "Integração Gateway",
            {
