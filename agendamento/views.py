...
def agenda_list(request):
    ...
    for painel in paineis:
        painel.inscrito = Agendamento.objects.filter(
            painel=painel, aluno=request.user, status="Agendado"
        ).exists()
...
def realizar_agendamento(request, painel_id):
    ...
    if not confirming_restriction_check(request, painel) and not confirmar_restricao:
        ...
        if conflitos:
            ...
            return render(
                request,
                "agendamento/confirm_restriction.html",
                {"painel": painel, "conflitos": conflitos, "restricoes_usuario": restricoes},
            )
...
def get_health_conflicts(user, painel):
    ...
    try:
        ...
        for item in painel.itens.all():
            ...
            if reasons:
                conflitos.append({"aula": aula, "categoria": " | ".join(reasons)})
