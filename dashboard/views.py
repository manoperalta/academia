...
def dashboard(request):
    ...
    if not (user.is_superuser or user.is_staff):
        ...
        ultimo_pagamento = (
            Pagamento.objects.filter(usuario=user).order_by("-data_pagamento").first()
        )
        if ultimo_pagamento:
    ...
    if user.is_superuser or user.is_staff:
        ...
        usuarios_ativos = User.objects.filter(
            is_active=True, is_staff=False, is_superuser=False, is_professor=False
        )
        ...
        for usuario in usuarios_ativos:
            ultimo_pag = (
                Pagamento.objects.filter(usuario=usuario, status="pago")
                .order_by("-data_fim")
                .first()
            )
            ...
            if ultimo_pag:
                if ultimo_pag.data_fim >= hoje:
                    ...
                    ultimo_pag.data_fim >= hoje - timedelta(days=dias_alerta)
                    and ultimo_pag.data_fim < hoje
                ):
                    usuarios_alerta += 1
                else:
        ...
        aniversariantes_hoje = Usuario.objects.filter(
            data_nasc__month=hoje.month, data_nasc__day=hoje.day, status_user="Ativo"
        )
        ...
        if hasattr(user, "professor_profile") and user.professor_profile.status_prof == "Ativo":
            ...
            total_alunos = Usuario.objects.filter(
                status_user="Ativo", user__is_staff=False, user__is_superuser=False
            ).count()
...
def enviar_notificacao_atraso(request):
    ...
    if request.method == "POST":
        ...
        for usuario in usuarios_ativos:
            ...
            if atrasado:
                ...
                if tipo_envio in ["whatsapp", "ambos"]:
                    ...
                    telefone = None
                    if (
                        hasattr(usuario, "usuario_profile")
                        and usuario.usuario_profile.telefone_user
                    ):
        ...
        messages.success(
            request, f"Processo de notificação finalizado. {enviados} notificações enviadas."
        )
        return redirect("dashboard")
