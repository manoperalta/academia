...
TELAS = [
    "plataforma:metricas", "plataforma:tenants", "plataforma:pacotes", "plataforma:faturas",
    "plataforma:regua", "plataforma:webhooks", "plataforma:relatorio", "plataforma:config",
]
...
def test_criar_tenant_pelo_painel(cliente_plataforma, pacote_prata):
    resposta = cliente_plataforma.post(reverse("plataforma:tenant_novo"), {
        "nome": "Academia Nova", "slug": "academia-nova", "pacote": pacote_prata.pk,
        "ciclo": "mensal", "cnpj": "12345678000199", "email": "dono@academia.com",
        "telefone": "51999999999", "trial": "on",
    })
    assert resposta.status_code == 302
...
def test_editar_cliente_com_excecao_comercial(cliente_plataforma, rede, assinatura, pacote_prata):
    resposta = cliente_plataforma.post(reverse("plataforma:tenant_editar", args=[rede.pk]), {
        "nome": "Academia Renomeada", "cnpj": "", "email_responsavel": "", "telefone": "",
        "dominio": "", "observacoes_internas": "Cliente estrategico",
        "pacote": pacote_prata.pk, "ciclo": "mensal", "limite_alunos_custom": 60,
...
def test_excecao_sem_motivo_e_recusada(cliente_plataforma, rede, assinatura, pacote_prata):
    resposta = cliente_plataforma.post(reverse("plataforma:tenant_editar", args=[rede.pk]), {
        "nome": rede.nome, "pacote": pacote_prata.pk, "ciclo": "mensal",
        "limite_alunos_custom": 60, "motivo_excecao": "",
    })
    assert resposta.status_code == 200
    assinatura.refresh_from_db()
...
def test_criar_e_editar_pacote(cliente_plataforma):
    resposta = cliente_plataforma.post(reverse("plataforma:pacote_novo"), {
        "nome": "Diamante", "codigo": "diamante", "descricao": "Top",
        "limite_alunos": 300, "limite_professores": 20, "limite_unidades": 5,
        "preco_mensal": "700.00", "preco_anual": "7000.00", "ordem_exibicao": 4,
        "visivel_no_site": "on", "ativo": "on", "modulos": ["api", "whatsapp"],
    })
    assert resposta.status_code == 302
...
def test_configuracao_salva_chave_do_gateway(cliente_plataforma):
    resposta = cliente_plataforma.post(reverse("plataforma:config"), {
        "nome_emitente": "SafeStack", "cnpj_emitente": "123", "email_financeiro": "fin@safestack.com.br",
        "asaas_api_key": "chave-de-teste", "asaas_ambiente": "sandbox", "asaas_base_url": "",
        "token_webhook": "token-de-teste", "trial_dias": 14, "dias_bloqueio": 10,
        "dias_suspensao": 30, "multa_percentual": "2", "juros_dia_percentual": "0.033",
        "regua_ativa": "on",
    })
    assert resposta.status_code == 302
