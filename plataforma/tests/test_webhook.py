...
def _payload(fatura, identificador_evento="evt-1", status="RECEIVED"):
    return {
        ...
        "payment": {
            "id": fatura.gateway_id or f"sim-{fatura.numero}",
            "status": status,
            "value": 150.0,
        },
    }
...
def _enviar(client, payload, token=None):
    ...
    return client.post(
        URL, data=json.dumps(payload), content_type="application/json", headers=cabecalhos
    )
...
def test_webhook_sem_id_de_evento_e_recusado(db, client):
    resposta = client.post(
        URL, data=json.dumps({"event": "PAYMENT_RECEIVED"}), content_type="application/json"
    )
...
def test_webhook_de_fatura_desconhecida_nao_quebra(db, client):
    resposta = _enviar(
        client, {"id": "evt-orfao", "payment": {"id": "nao-existe", "status": "RECEIVED"}}
    )
