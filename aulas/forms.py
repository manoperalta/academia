...
class AulasForm(forms.ModelForm):
    class Meta:
        model = Aulas
        fields = ["nome", "descricao", "file_de_video", "categorias_exercicios", "restricao"]
        widgets = {
            "nome": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "Título da Aula"}
            ),
            ...
            "restricao": forms.Select(attrs={"class": "form-select"}),
        }
...
ImagemAulaFormSet = inlineformset_factory(
    Aulas, ImagemAula, form=ImagemAulaForm, extra=1, can_delete=True, max_num=5
)
