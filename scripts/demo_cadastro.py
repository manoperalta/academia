...
print(
    f"   plano escolhido          -> {pacotes['prata'].nome} R$ {pacotes['prata'].preco_mensal}/mes"
)
...
def cadastrar(marcador: str, modalidade: str, pacote, deslocamento: int = 0) -> dict:
    ...
    print(
        f"   tenant: {rede.nome} ({rede.slug}) | status={rede.status} | pacote={assinatura.pacote.nome}"
    )
...
if pago.get("fatura"):
    ...
    primeira = publico.post(
        "/plataforma/webhook/asaas/",
        data=evento,
        content_type="application/json",
        headers=cabecalhos,
    )
    segunda = publico.post(
        "/plataforma/webhook/asaas/",
        data=evento,
        content_type="application/json",
        headers=cabecalhos,
    ...
    print(
        f"   1a notificacao: HTTP {primeira.status_code} processado={primeira.json()['processado']}"
    )
    print(
        f"   2a notificacao: HTTP {segunda.status_code} processado={segunda.json()['processado']} "
...
if trial.get("link"):
    ...
    aceite = novo.post(
        caminho,
        {"nome": "Dono da Academia", "senha": "SenhaForteDemo123", "senha2": "SenhaForteDemo123"},
    )
...
criados = Rede.todos.filter(slug__startswith="demo-", slug__endswith=sufixo)
