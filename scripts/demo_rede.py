...
unidades = [
    ...
    for nome, tipo, cidade in (
        ("Centro", "propria", "Montenegro"),
...
for indice, unidade in enumerate(unidades):
    ...
    login = get_user_model().objects.create_user(
        username=f"demo-pend-{sufixo}-{indice}",
        password=secrets.token_urlsafe(12),
...
regra = RegraDeRepasse.objects.create(
    rede=rede,
    unidade=None,
    tipo=RegraDeRepasse.Tipo.PERCENTUAL,
    percentual=Decimal("8.000"),
    base=RegraDeRepasse.Base.LIQUIDO,
    percentual_de_taxas_de_gateway=Decimal("2.50"),
    excluir_taxas_de_gateway=True,
    fundo_de_marketing=Decimal("1.50"),
    piso_minimo=Decimal("50.00"),
)
print(
...
for item in primeiro.itens.all():
    print(
        f"   {item.ordem}. [{item.get_tipo_display():22s}] {item.descricao[:52]:52s} "
        f"base R$ {item.base:>8} x {item.percentual:>6}% = R$ {item.valor:>8}"
    )
...
for linha in comparativo:
    print(
        f"   {linha['posicao']}o {linha['unidade'].nome:12s} recebido R$ {linha['recebido']:>8} · "
...
print(
    f"   desconto 5% -> permitido={dentro['permitido']} · desconto 30% -> "
...
print(
    f"\n9) BAIXA REGISTRADA: situacao {primeiro.get_situacao_display()}, saldo R$ {primeiro.saldo}"
)
