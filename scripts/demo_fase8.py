...
professor = Professor.todos.create(
    rede=rede, unidade=unidade, nome="Prof. Demo", status_prof="Ativo"
)
...
print(
    "   conferencia por recalculo:",
    "CONFERE" if conferencia["ok"] else "DIVERGE",
    "| mesmo numero:",
    conferencia["mesmo_numero"],
    "| hash:",
    apuracao.hash_do_calculo[:12],
)
...
print(
    "   check-in:",
    checkin.unidade.nome,
    "| pontos ganhos:",
    ganho.get("pontos"),
    "| conquistas:",
    [c["nome"] for c in ganho.get("conquistas", [])],
)
print(
    "   saldo:", saldo_do_aluno(aluno)["pontos"], "pontos | nivel", saldo_do_aluno(aluno)["nivel"]
)
print(
    "   ranking:", [(item["posicao"], item["nome"], item["pontos"]) for item in ranking(rede=rede)]
)
