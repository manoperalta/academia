...
def patch_settings():
    ...
    if 'MEDIA_URL = "/midia/"' not in texto:
        novo, trocas = re.subn(
            r"^MEDIA_URL = .*$", 'MEDIA_URL = "/midia/"', texto, count=1, flags=re.MULTILINE
        )
        if trocas == 0:
    ...
    if "governanca.middleware.DoisFatoresMiddleware" not in texto:
        ...
        achou = re.search(
            r'^\s*["\']core\.middleware\.ResolucaoDeTenantMiddleware["\'],?\s*$',
            texto,
            re.MULTILINE,
        )
        if achou:
...
def patch_tenancy():
    ...
    if "rede_por_host" in texto:
        ...
        return
    padrao = re.compile(r'(def resolver_rede\(request\):\s*\n(?:\s*"""[\s\S]*?"""\s*\n)?)')
...
def patch_pyproject():
    ...
    novo, trocas = re.subn(
        r"(testpaths\s*=\s*\[)([^\]]*)(\])",
        lambda m: m.group(1) + m.group(2) + ', "governanca"' + m.group(3),
        texto,
        count=1,
    )
    if trocas == 0:
...
def patch_apoio_de_testes():
    ...
    if '"governanca"' in texto:
        ...
        return
    novo, trocas = re.subn(
        r'("contenttypes",\s*"core")', r'"contenttypes", "core", "governanca"', texto, count=1
    )
    if trocas == 0:
