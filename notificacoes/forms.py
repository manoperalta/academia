    ...
    class Meta:
        ...
        widgets = {
            "assunto": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "Digite o assunto do e-mail"}
            ),
            "mensagem": forms.Textarea(
                attrs={"class": "form-control", "rows": 4, "placeholder": "Digite a mensagem..."}
            ),
            "destinatarios": forms.Textarea(
                attrs={
                    "class": "form-control",
                    "rows": 2,
    ...
    def clean(self):
        ...
        if not enviar_todos and not destinatarios:
            raise forms.ValidationError(
                "Informe os destinatários ou selecione 'Enviar para todos'."
            )
        ...
        if enviar_todos:
            ...
            usuario_emails = Usuario.objects.filter(status_user="Ativo").values_list(
                "email_user", flat=True
            )
            emails.update([e for e in usuario_emails if e])

            # E-mails dos perfis de Professor
            prof_emails = Professor.objects.filter(status_prof="Ativo").values_list(
                "email_prof", flat=True
            )
