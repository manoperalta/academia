...
class ConfiguracaoEmailAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Servidor SMTP", {"fields": ("host", "port", "use_tls", "use_ssl")}),
        ("Autenticação", {"fields": ("username", "password")}),
        ("Remetente", {"fields": ("remetente_nome", "remetente_email", "ativo")}),
    )
...
class NotificacaoAdmin(admin.ModelAdmin):
    ...
    def enviar_notificacoes(self, request, queryset):
        ...
        self.message_user(
            request, f"{enviados} notificações enviadas com sucesso. {falhas} falhas."
        )
