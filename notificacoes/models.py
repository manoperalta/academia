...
class ConfiguracaoEmail(TenantModel):
    ...
    remetente_nome = models.CharField(
        max_length=255, default="Academia System", verbose_name="Nome do Remetente"
    )
...
class ConfiguracaoWhatsapp(TenantModel):
    ...
    business_account_id = models.CharField(
        max_length=100, verbose_name="WhatsApp Business Account ID", blank=True, null=True
    )
...
class Notificacao(TenantModel):
    ...
    destinatarios = models.TextField(
        verbose_name="Destinatários", help_text="Emails ou números separados por vírgula"
    )
    tipo = models.CharField(
        max_length=20, choices=TIPO_CHOICES, default="email", verbose_name="Tipo"
    )

    criado_por = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, verbose_name="Criado por"
    )
